...
function writeValue(s, ctx) {
    ...
    if (typeof s === "string") {
        const id = (0, writeConstant_1.writeString)(s, ctx);
        ctx.used(id);
        return `${id}()`;
    }
    if (typeof s === "boolean") {
    ...
    if (core_1.Address.isAddress(s)) {
        const res = (0, writeConstant_1.writeAddress)(s, ctx);
        ctx.used(res);
        return res + "()";
    }
    if (s instanceof core_1.Cell) {
        const res = (0, writeConstant_1.writeCell)(s, ctx);
        ctx.used(res);
        return `${res}()`;
    }
    if (s === null) {
...
function writeExpression(f, ctx) {
    ...
    // Try intrinsics
    //
    const intrinsic = (0, tryExpressionIntrinsics_1.tryExpressionIntrinsics)(f, ctx);
    if (intrinsic) {
        return intrinsic;
    }
    //
    ...
    //
    if (f.kind === "id") {
        const t = (0, resolveExpression_1.getExpType)(ctx.ctx, f);
        // Handle packed type
        if (t.kind === "ref") {
        ...
        if ((0, resolveDescriptors_1.hasStaticConstant)(ctx.ctx, f.value)) {
            const c = (0, resolveDescriptors_1.getStaticConstant)(ctx.ctx, f.value);
    ...
    if (f.kind === "op_binary") {
        ...
        const lt = (0, resolveExpression_1.getExpType)(ctx.ctx, f.left);
        const rt = (0, resolveExpression_1.getExpType)(ctx.ctx, f.right);
        ...
        return ("(" +
            writeExpression(f.left, ctx) +
            " " +
            op +
            " " +
            writeExpression(f.right, ctx) +
            ")");
    }
    //
    // Unary operations: !, -, +, !!
    ...
    if (f.kind === "op_unary") {
        ...
        if (f.op === "!!") {
            const t = (0, resolveExpression_1.getExpType)(ctx.ctx, f.right);
            if (t.kind === "ref") {
                const tt = (0, resolveDescriptors_1.getType)(ctx.ctx, t.name);
                if (tt.kind === "struct") {
    ...
    if (f.kind === "op_field") {
        ...
        const src = (0, resolveExpression_1.getExpType)(ctx.ctx, f.src);
        if (src === null ||
        ...
        const srcT = (0, resolveDescriptors_1.getType)(ctx.ctx, src.name);
        // Resolve field
        let fields;
        fields = srcT.fields;
        ...
        if (field) {
            ...
            if (path) {
                ...
                if (field.type.kind === "ref") {
                    const ft = (0, resolveDescriptors_1.getType)(ctx.ctx, field.type.name);
    ...
    if (f.kind === "op_static_call") {
        ...
        if (global_1.GlobalFunctions.has(f.name)) {
            return global_1.GlobalFunctions.get(f.name).generate(ctx, f.args.map((v) => (0, resolveExpression_1.getExpType)(ctx.ctx, v)), f.args, f.ref);
        }
        const sf = (0, resolveDescriptors_1.getStaticFunction)(ctx.ctx, f.name);
        ...
        return (n +
            "(" +
            f.args
                .map((a, i) => (0, writeFunction_1.writeCastedExpression)(a, sf.args[i].type, ctx))
                .join(", ") +
            ")");
    }
    //
    ...
    //
    if (f.kind === "op_new") {
        const src = (0, resolveDescriptors_1.getType)(ctx.ctx, f.type);
        // Write a constructor
        const id = writeStructConstructor(src, f.args.map((v) => v.name), ctx);
        ctx.used(id);
        // Write an expression
        const expressions = f.args.map((v) => (0, writeFunction_1.writeCastedExpression)(v.exp, src.fields.find((v2) => v2.name === v.name).type, ctx), ctx);
    ...
    if (f.kind === "op_call") {
        ...
        if (src.kind === "ref") {
            ...
            const t = (0, resolveDescriptors_1.getType)(ctx.ctx, src.name);
            // Check struct ABI
            if (t.kind === "struct") {
            ...
            if (ff.isMutating) {
                if (f.args.length === 1) {
                    const t = (0, resolveExpression_1.getExpType)(ctx.ctx, f.args[0]);
                    if (t.kind === "ref") {
                        const tt = (0, resolveDescriptors_1.getType)(ctx.ctx, t.name);
                        if ((tt.kind === "contract" || tt.kind === "struct") &&
                            ...
                            renderedArguments = [
                                `${ops_1.ops.typeTensorCast(tt.name, ctx)}(${renderedArguments[0]})`,
                            ];
                        }
                    }
                }
            ...
            if (ff.isMutating) {
                if (f.src.kind === "id" || f.src.kind === "op_field") {
                    ...
                    return `${ctx.used(ops_1.ops.nonModifying(name))}(${[s, ...renderedArguments].join(", ")})`;
                }
            }
            else {
    ...
    //
    if (f.kind === "init_of") {
        const type = (0, resolveDescriptors_1.getType)(ctx.ctx, f.name);
