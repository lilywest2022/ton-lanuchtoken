...
function writeStatement(f, self, returns, ctx) {
    if (f.kind === "statement_return") {
        ...
        const t = (0, resolveDescriptors_1.resolveTypeRef)(ctx.ctx, f.type);
        if (t.kind === "ref") {
        ...
        if (t.key === "Int") {
            ...
            if (t.value === "Int") {
                ...
                ctx.append(`var (${(0, id_1.id)(f.keyName)}, ${(0, id_1.id)(f.valueName)}, ${flag}) = ${ctx.used(`__tact_dict_min_${kind}_${vKind}`)}(${(0, id_1.id)(f.map.value)}, ${bits}, ${vBits});`);
                ctx.append(`while (${flag}) {`);
                ctx.inIndent(() => {
                    for (const s of f.statements) {
                        writeStatement(s, self, returns, ctx);
                    }
                    ctx.append(`(${(0, id_1.id)(f.keyName)}, ${(0, id_1.id)(f.valueName)}, ${flag}) = ${ctx.used(`__tact_dict_next_${kind}_${vKind}`)}(${(0, id_1.id)(f.map.value)}, ${bits}, ${(0, id_1.id)(f.keyName)}, ${vBits});`);
                });
                ctx.append(`}`);
            }
            else if (t.value === "Bool") {
                ctx.append(`var (${(0, id_1.id)(f.keyName)}, ${(0, id_1.id)(f.valueName)}, ${flag}) = ${ctx.used(`__tact_dict_min_${kind}_int`)}(${(0, id_1.id)(f.map.value)}, ${bits}, 1);`);
                ctx.append(`while (${flag}) {`);
                ctx.inIndent(() => {
                    for (const s of f.statements) {
                        writeStatement(s, self, returns, ctx);
                    }
                    ctx.append(`(${(0, id_1.id)(f.keyName)}, ${(0, id_1.id)(f.valueName)}, ${flag}) = ${ctx.used(`__tact_dict_next_${kind}_int`)}(${(0, id_1.id)(f.map.value)}, ${bits}, ${(0, id_1.id)(f.keyName)}, 1);`);
                });
                ctx.append(`}`);
            }
            else if (t.value === "Cell") {
                ctx.append(`var (${(0, id_1.id)(f.keyName)}, ${(0, id_1.id)(f.valueName)}, ${flag}) = ${ctx.used(`__tact_dict_min_${kind}_cell`)}(${(0, id_1.id)(f.map.value)}, ${bits});`);
                ctx.append(`while (${flag}) {`);
                ctx.inIndent(() => {
                    for (const s of f.statements) {
                        writeStatement(s, self, returns, ctx);
                    }
                    ctx.append(`(${(0, id_1.id)(f.keyName)}, ${(0, id_1.id)(f.valueName)}, ${flag}) = ${ctx.used(`__tact_dict_next_${kind}_cell`)}(${(0, id_1.id)(f.map.value)}, ${bits}, ${(0, id_1.id)(f.keyName)});`);
                });
                ctx.append(`}`);
            }
            else if (t.value === "Address") {
                ctx.append(`var (${(0, id_1.id)(f.keyName)}, ${(0, id_1.id)(f.valueName)}, ${flag}) = ${ctx.used(`__tact_dict_min_${kind}_slice`)}(${(0, id_1.id)(f.map.value)}, ${bits});`);
                ctx.append(`while (${flag}) {`);
                ctx.inIndent(() => {
                    for (const s of f.statements) {
                        writeStatement(s, self, returns, ctx);
                    }
                    ctx.append(`(${(0, id_1.id)(f.keyName)}, ${(0, id_1.id)(f.valueName)}, ${flag}) = ${ctx.used(`__tact_dict_next_${kind}_slice`)}(${(0, id_1.id)(f.map.value)}, ${bits}, ${(0, id_1.id)(f.keyName)});`);
                });
                ctx.append(`}`);
            }
            else {
                ...
                ctx.append(`while (${flag}) {`);
                ctx.inIndent(() => {
                    ctx.append(`var ${(0, resolveFuncTypeUnpack_1.resolveFuncTypeUnpack)(t.value, (0, id_1.id)(f.valueName), ctx)} = ${ops_1.ops.typeNotNull(t.value, ctx)}(${ops_1.ops.readerOpt(t.value, ctx)}(${(0, id_1.id)(f.valueName)}));`);
                    for (const s of f.statements) {
                        writeStatement(s, self, returns, ctx);
                    }
                    ctx.append(`(${(0, id_1.id)(f.keyName)}, ${(0, id_1.id)(f.valueName)}, ${flag}) = ${ctx.used(`__tact_dict_next_${kind}_cell`)}(${(0, id_1.id)(f.map.value)}, ${bits}, ${(0, id_1.id)(f.keyName)});`);
        ...
        if (t.key === "Address") {
            if (t.value === "Int") {
                ...
                ctx.append(`var (${(0, id_1.id)(f.keyName)}, ${(0, id_1.id)(f.valueName)}, ${flag}) = ${ctx.used(`__tact_dict_min_slice_${vKind}`)}(${(0, id_1.id)(f.map.value)}, 267, ${vBits});`);
                ctx.append(`while (${flag}) {`);
                ctx.inIndent(() => {
                    for (const s of f.statements) {
                        writeStatement(s, self, returns, ctx);
                    }
                    ctx.append(`(${(0, id_1.id)(f.keyName)}, ${(0, id_1.id)(f.valueName)}, ${flag}) = ${ctx.used(`__tact_dict_next_slice_${vKind}`)}(${(0, id_1.id)(f.map.value)}, 267, ${(0, id_1.id)(f.keyName)}, ${vBits});`);
                });
                ctx.append(`}`);
            }
            else if (t.value === "Bool") {
                ctx.append(`var (${(0, id_1.id)(f.keyName)}, ${(0, id_1.id)(f.valueName)}, ${flag}) = ${ctx.used(`__tact_dict_min_slice_int`)}(${(0, id_1.id)(f.map.value)}, 267, 1);`);
                ctx.append(`while (${flag}) {`);
                ctx.inIndent(() => {
                    for (const s of f.statements) {
                        writeStatement(s, self, returns, ctx);
                    }
                    ctx.append(`(${(0, id_1.id)(f.keyName)}, ${(0, id_1.id)(f.valueName)}, ${flag}) = ${ctx.used(`__tact_dict_next_slice_int`)}(${(0, id_1.id)(f.map.value)}, 267, ${(0, id_1.id)(f.keyName)}, 1);`);
                });
                ctx.append(`}`);
            }
            else if (t.value === "Cell") {
                ctx.append(`var (${(0, id_1.id)(f.keyName)}, ${(0, id_1.id)(f.valueName)}, ${flag}) = ${ctx.used(`__tact_dict_min_slice_cell`)}(${(0, id_1.id)(f.map.value)}, 267);`);
                ctx.append(`while (${flag}) {`);
                ctx.inIndent(() => {
                    for (const s of f.statements) {
                        writeStatement(s, self, returns, ctx);
                    }
                    ctx.append(`(${(0, id_1.id)(f.keyName)}, ${(0, id_1.id)(f.valueName)}, ${flag}) = ${ctx.used(`__tact_dict_next_slice_cell`)}(${(0, id_1.id)(f.map.value)}, 267, ${(0, id_1.id)(f.keyName)});`);
                });
                ctx.append(`}`);
            }
            else if (t.value === "Address") {
                ctx.append(`var (${(0, id_1.id)(f.keyName)}, ${(0, id_1.id)(f.valueName)}, ${flag}) = ${ctx.used(`__tact_dict_min_slice_slice`)}(${(0, id_1.id)(f.map.value)}, 267);`);
                ctx.append(`while (${flag}) {`);
                ctx.inIndent(() => {
                    for (const s of f.statements) {
                        writeStatement(s, self, returns, ctx);
                    }
                    ctx.append(`(${(0, id_1.id)(f.keyName)}, ${(0, id_1.id)(f.valueName)}, ${flag}) = ${ctx.used(`__tact_dict_next_slice_slice`)}(${(0, id_1.id)(f.map.value)}, 267, ${(0, id_1.id)(f.keyName)});`);
                });
                ctx.append(`}`);
            }
            else {
