    ...
    [
        "del",
        {
            ...
            resolve(ctx, args, ref) {
                // Check arguments
                if (args.length !== 2) {
                    (0, ast_1.throwError)("del expects one argument", ref); // Ignore self argument
                }
                const self = args[0];
                if (!self || self.kind !== "map") {
                    (0, ast_1.throwError)("del expects a map as self argument", ref); // Should not happen
                }
                // Check key type
                if (args[1].kind !== "ref" || args[1].optional) {
                    (0, ast_1.throwError)("del expects a direct type as first argument", ref);
                }
                if (args[1].name !== self.key) {
                    (0, ast_1.throwError)(`del expects a "${self.key}" as first argument`, ref);
                }
                // Returns boolean
            ...
            generate: (ctx, args, exprs, ref) => {
                ...
                (0, ast_1.throwError)(`del expects a map with Int keys`, ref);
            },
        },
    ],
    [
    ...
    [
        "isEmpty",
        {
            ...
            resolve(ctx, args, ref) {
                // Check arguments
                if (args.length !== 1) {
                    (0, ast_1.throwError)("isEmpty expects one argument", ref); // Ignore self argument
                }
                const self = args[0];
                if (!self || self.kind !== "map") {
                    (0, ast_1.throwError)("isEmpty expects a map as self argument", ref); // Should not happen
                }
                return { kind: "ref", name: "Bool", optional: false };
            },
            generate: (ctx, args, exprs, ref) => {
