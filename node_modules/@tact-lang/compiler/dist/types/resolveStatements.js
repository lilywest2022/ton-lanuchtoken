    ...
    for (const s of statements) {
        ...
        if (s.kind === "statement_let") {
            ...
            if (sctx.vars.has(s.name)) {
                (0, ast_1.throwError)(`Variable "${s.name}" already exists`, s.ref);
            }
            ...
            if (expressionType.kind !== "ref" ||
                ...
                tailType.name !== "Int" ||
                tailType.optional) {
                (0, ast_1.throwError)(`Type error: Augmented assignment is only allowed for Int type`, s.ref);
            }
            // Mark as assigned
            if (s.path.length === 2 && s.path[0].name === "self") {
                const field = s.path[1].name;
            ...
            if (sctx.requiredFields.length > 0) {
                if (sctx.requiredFields.length === 1) {
                    (0, ast_1.throwError)(`Field "${sctx.requiredFields[0]}" is not set`, sctx.root);
                }
                else {
                    (0, ast_1.throwError)(`Fields ${sctx.requiredFields.map((x) => '"' + x + '"').join(", ")} are not set`, sctx.root);
                }
            }
            ...
            if (initialCtx.vars.has(s.catchName)) {
                (0, ast_1.throwError)(`Variable already exists: "${s.catchName}"`, s.ref);
            }
            let catchCtx = addVariable(s.catchName, { kind: "ref", name: "Int", optional: false }, initialCtx);
            ...
            if (!mapVariable || mapVariable.kind !== "map") {
                (0, ast_1.throwError)(`Variable "${s.map.value}" is not a map`, s.ref);
            }
            // Add key and value to statement context
            if (initialCtx.vars.has(s.keyName)) {
                (0, ast_1.throwError)(`Variable already exists: "${s.keyName}"`, s.ref);
            }
            let foreachCtx = addVariable(s.keyName, { kind: "ref", name: mapVariable.key, optional: false }, initialCtx);
            if (foreachCtx.vars.has(s.valueName)) {
                (0, ast_1.throwError)(`Variable already exists: "${s.valueName}"`, s.ref);
            }
            foreachCtx = addVariable(s.valueName, { kind: "ref", name: mapVariable.value, optional: false }, foreachCtx);
            // Process inner statements
...
function processFunctionBody(statements, sctx, ctx) {
    ...
    if (sctx.returns.kind !== "void" && !res.returnAlwaysReachable) {
        (0, ast_1.throwError)(`Function does not always return a result. Adding 'return' statement(s) should fix the issue.`, res.sctx.root);
    }
    // Check if all required variables are assigned
    if (res.sctx.requiredFields.length > 0) {
        if (res.sctx.requiredFields.length === 1) {
            (0, ast_1.throwError)(`Field "${res.sctx.requiredFields[0]}" is not set`, res.sctx.root);
        }
        else {
            (0, ast_1.throwError)(`Fields ${res.sctx.requiredFields.map((x) => '"' + x + '"').join(", ")} are not set`, res.sctx.root);
        }
    }
    return res.ctx;
}
    ...
    for (const t of Object.values((0, resolveDescriptors_1.getAllTypes)(ctx))) {
        ...
        if (t.init) {
            ...
            for (const f of t.fields) {
                ...
                if ((0, isAssignable_1.isAssignable)({ kind: "null" }, f.type)) {
                    continue;
                }
