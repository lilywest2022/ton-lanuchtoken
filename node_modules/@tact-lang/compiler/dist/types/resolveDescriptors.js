    ...
    if (keyAs) {
        if (key === "Int") {
            if (![
                ...
                (0, ast_1.throwError)("Invalid key type for map", ref);
            }
        }
        else {
            (0, ast_1.throwError)("Invalid key type for map", ref);
    ...
    if (valueAs) {
        if (value === "Int") {
            if (![
                ...
                (0, ast_1.throwError)("Invalid value type for map", ref);
            }
        }
        else {
            (0, ast_1.throwError)("Invalid value type for map", ref);
...
function resolveDescriptors(ctx) {
    ...
    function buildFieldDescription(src, index) {
        ...
        if ((0, isRuntimeType_1.isRuntimeType)(tr)) {
            (0, ast_1.throwError)((0, types_1.printTypeRef)(tr) +
                " is a runtime only type and can't be used as field", src.ref);
        }
        // Resolve default value
        let d = undefined;
        if (src.init) {
            d = (0, resolveConstantValue_1.resolveConstantValue)(tr, src.init, ctx);
        }
        // Resolve abi type
    ...
    function buildConstantDescription(src) {
        const tr = buildTypeRef(src.type, types);
        const d = (0, resolveConstantValue_1.resolveConstantValue)(tr, src.value, ctx);
    ...
    for (const a of ast.types) {
            ...
            for (const f of a.declarations) {
                if (f.kind === "def_field") {
                    ...
                    if (f.attributes.find((v) => v.type !== "overrides")) {
                        (0, ast_1.throwError)(`Constant can be only overridden`, f.ref);
                    }
                    types
                        .get(a.name)
        ...
        if (a.kind === "def_struct") {
            ...
            if (a.fields.length === 0 && !a.message) {
                (0, ast_1.throwError)(`Struct "${a.name}" must have at least one field`, a.ref);
            }
        }
        // Trait
        if (a.kind === "def_trait") {
            for (const f of a.declarations) {
                if (f.kind === "def_field") {
                    ...
                    if (f.attributes.find((v) => v.type === "overrides")) {
                        (0, ast_1.throwError)(`Trait constant cannot be overridden`, f.ref);
                    }
    ...
    function resolveFunctionDescriptor(optSelf, a, origin) {
        ...
        if (a.kind === "def_native_function") {
            ...
            if (self) {
                (0, ast_1.throwError)("Native functions cannot be defined within a contract", a.ref);
            }
            if (isVirtual) {
                (0, ast_1.throwError)("Native functions cannot be virtual", isVirtual.ref);
            }
            if (isOverrides) {
                (0, ast_1.throwError)("Native functions cannot be overrides", isOverrides.ref);
            }
        }
        ...
        if (isOverrides && isExtends) {
            (0, ast_1.throwError)("Extend functions cannot be overrides", isOverrides.ref);
        }
        if (isAbstract && isExtends) {
            (0, ast_1.throwError)("Extend functions cannot be abstract", isAbstract.ref);
        }
        if (!self && isVirtual) {
            (0, ast_1.throwError)("Virtual functions must be defined within a contract or a trait", isVirtual.ref);
        }
        if (!self && isOverrides) {
            (0, ast_1.throwError)("Overrides functions must be defined within a contract or a trait", isOverrides.ref);
        }
        if (!self && isAbstract) {
            (0, ast_1.throwError)("Abstract functions must be defined within a trait", isAbstract.ref);
        }
        if (isVirtual && isAbstract) {
            (0, ast_1.throwError)("Abstract functions cannot be virtual", isAbstract.ref);
        }
        if (isVirtual && isOverrides) {
            (0, ast_1.throwError)("Overrides functions cannot be virtual", isOverrides.ref);
        }
        if (isAbstract && isOverrides) {
            (0, ast_1.throwError)("Overrides functions cannot be abstract", isOverrides.ref);
        }
        // Check virtual
        if (isVirtual) {
        ...
        if (isOverrides) {
            const t = types.get(self);
            if (t.kind !== "contract") {
                (0, ast_1.throwError)("Overrides functions must be defined within a contract", isOverrides.ref);
            }
        }
        // Check for common
        ...
        if (isInline && isGetter) {
            (0, ast_1.throwError)("Getters cannot be inline", isInline.ref);
        }
        // Validate mutating
        if (isExtends) {
            // Validate arguments
    ...
    function resolveInitFunction(ast) {
        ...
        for (const a of args) {
            if ((0, isRuntimeType_1.isRuntimeType)(a.type)) {
                (0, ast_1.throwError)((0, types_1.printTypeRef)(a.type) +
            ...
            for (const d of a.declarations) {
                ...
                if (d.kind === "def_receive") {
                    ...
                    if (d.selector.kind.startsWith("external-") &&
                        ...
                        (0, ast_1.throwError)("External functions are not enabled", d.ref);
                    }
                    if (d.selector.kind === "internal-simple" ||
                        ...
                        if (arg.type.kind === "type_ref_simple") {
                            if (arg.type.optional) {
                                (0, ast_1.throwError)("Bounce receive function cannot have optional argument", d.ref);
                            }
                            if (arg.type.name === "Slice") {
                                if (s.receivers.find((v) => v.selector.kind ===
                                    "bounce-fallback")) {
                                    (0, ast_1.throwError)(`Fallback bounce receive function already exists`, d.ref);
                                }
                                s.receivers.push({
                                    selector: {
                                ...
                                if (type.ast.kind !== "def_struct" ||
                                    !type.ast.message) {
                                    (0, ast_1.throwError)("Bounce receive function can only accept bounced message, message or Slice", d.ref);
                                }
                                if (type.fields.length !==
                                    type.partialFieldCount) {
                                    (0, ast_1.throwError)("This message is too big for bounce receiver, you need to wrap it to a bounced<" +
                                        arg.type.name +
                                        ">.", d.ref);
                                }
                                if (s.receivers.find((v) => v.selector.kind ===
                                    ...
                                    (0, ast_1.throwError)(`Bounce receive function for "${arg.type.name}" already exists`, d.ref);
                                }
                                s.receivers.push({
                                    selector: {
                                        kind: "bounce-binary",
                            ...
                            const t = types.get(arg.type.name);
                            if (t.kind !== "struct") {
                                (0, ast_1.throwError)("Bounce receive function can only accept bounced<T> struct types", d.ref);
                            }
                            if (t.ast.kind !== "def_struct") {
                                (0, ast_1.throwError)("Bounce receive function can only accept bounced<T> struct types", d.ref);
                            }
                            ...
                            if (s.receivers.find((v) => v.selector.kind === "bounce-binary" &&
                                ...
                                (0, ast_1.throwError)(`Bounce receive function for "${t.name}" already exists`, d.ref);
                            }
                            if (t.fields.length === t.partialFieldCount) {
                                (0, ast_1.throwError)("This message is small enough for bounce receiver, you need to remove bounced modifier.", d.ref);
                            }
                            s.receivers.push({
                                selector: {
                                    kind: "bounce-binary",
                            ...
                            (0, ast_1.throwError)("Bounce receive function can only accept bounced<T> struct args or Slice", d.ref);
                        }
                    }
                    else {
                        (0, ast_1.throwError)("Invalid receive function selector", d.ref);
                    }
                }
            }
        }
        ...
        if (t.ast.kind === "def_trait" || t.ast.kind === "def_contract") {
            ...
            function visit(name) {
                ...
                if (tt.ast.kind === "def_trait") {
                    ...
                    (0, ast_1.throwError)("Type " + name + " is not a trait", t.ast.ref);
                }
            }
            visit("BaseTrait");
    ...
    function copyTraits(t) {
        for (const tr of t.traits) {
            ...
            for (const f of tr.functions.values()) {
                ...
                if (!ex && f.isAbstract) {
                    (0, ast_1.throwError)(`Trait "${tr.name}" requires function "${f.name}"`, t.ast.ref);
                }
                // Check overrides
                if (ex && ex.isOverrides) {
                    if (f.isGetter) {
                        (0, ast_1.throwError)(`Overridden function "${f.name}" can not be a getter`, ex.ast.ref);
                    }
                    if (f.isMutating !== ex.isMutating) {
                        (0, ast_1.throwError)(`Overridden function "${f.name}" should have same mutability`, ex.ast.ref);
                    }
                    if (!(0, types_1.typeRefEquals)(f.returns, ex.returns)) {
                        (0, ast_1.throwError)(`Overridden function "${f.name}" should have same return type`, ex.ast.ref);
                    }
                    if (f.args.length !== ex.args.length) {
                        (0, ast_1.throwError)(`Overridden function "${f.name}" should have same number of arguments`, ex.ast.ref);
                    }
                    for (let i = 0; i < f.args.length; i++) {
                        const a = ex.args[i];
                        const b = f.args[i];
                        if (!(0, types_1.typeRefEquals)(a.type, b.type)) {
                            (0, ast_1.throwError)(`Overridden function "${f.name}" should have same argument types`, ex.ast.ref);
                        }
                    }
            ...
            for (const f of tr.constants) {
                ...
                if (!ex &&
                    ...
                    (0, ast_1.throwError)(`Trait "${tr.name}" requires constant "${f.name}"`, t.ast.ref);
                }
                // Check overrides
                if (ex &&
                    ex.ast.attributes.find((v) => v.type === "overrides")) {
                    if (!(0, types_1.typeRefEquals)(f.type, ex.type)) {
                        (0, ast_1.throwError)(`Overridden constant "${f.name}" should have same type`, ex.ast.ref);
                    }
                    continue;
                }
                // Check duplicates
                if (ex) {
                    (0, ast_1.throwError)(`Constant "${f.name}" already exist in "${t.name}"`, t.ast.ref);
                }
                // Register constant
                t.constants.push({
                    ...f,
    ...
    function processType(name) {
        ...
        if (processing.has(name)) {
            (0, ast_1.throwError)(`Circular dependency detected for type "${name}"`, types.get(name).ast.ref);
        }
        processing.has(name);
